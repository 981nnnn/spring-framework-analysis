...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * 	singletonFactories
	 * 		中缓存的是ObjectFactory，表示对象工厂，表示用来创建早期bean对象的工厂。
	 */
	 ...
	 *
	 * <p>
	 * AOP可以说是Spring中除开IOC的另外一大功能，而循环依赖又是属于IOC范畴的，
	 * 所以这两大功能想要并存，Spring需要特殊处理。
	 *
	 * 首先，singletonFactories中存的是【某个beanName对应的ObjectFactory】，在bean的生命周期中，生成完原始对象之后，就会构造一个ObjectFactory存入singletonFactories中。
	 * 这个ObjectFactory是一个函数式接口，所以支持Lambda表达式：() -> getEarlyBeanReference(beanName, mbd, bean)
	 *
	 * 执行这个lambda表达式，就是AbstractAutoProxyCreator中的getEarlyBeanReference方法
	 * <pre>
	 ...
	 *
	 * <p>
	 *     三级缓存singletonFactories存在意义：
	 *     只有在出现了循环依赖的情况，才会执行lambda表达式，才会进行AOP，也就说只有在出现了循环依赖的情况下才会打破Bean生命周期的设计，
	 *     如果一个Bean没有出现循环依赖，那么它还是遵守了Bean的生命周期的设计的。
	 *  */
